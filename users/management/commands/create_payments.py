...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        user_2, _ = User.objects.get_or_create(
            email="user2@mail.ru", defaults={"is_active": True}
        )
        user_3, _ = User.objects.get_or_create(
            email="user3@mail.ru", defaults={"is_active": True}
        )
        user_4, _ = User.objects.get_or_create(
            email="user4@mail.ru", defaults={"is_active": True}
        )

        course_web, _ = Course.objects.get_or_create(
            title="веб-разработчик",
            defaults={"description": "Описание веб-разработчика"},
        )
        course_python, _ = Course.objects.get_or_create(
            title="python-разработчик",
            defaults={"description": "Описание python-разработчика"},
        )
        qr_engineer, _ = Course.objects.get_or_create(
            title="тестировщик", defaults={"description": "Описание тестировщика"}
        )
